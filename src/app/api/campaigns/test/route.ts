...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: parsed.error.issues[0]?.message ?? "Revise os campos da campanha." },
      { status: 400 }
    );
  }
  ...
  if (importResult.contacts.length === 0) {
    return NextResponse.json(
      { error: "Nenhum telefone valido com DDD foi encontrado." },
      { status: 400 }
    );
  }
  ...
  if (importResult.contacts.length > 50) {
    return NextResponse.json(
      { error: "Campanha de teste aceita ate 50 numeros por vez." },
      { status: 400 }
    );
  }
  ...
  if (headerFile instanceof File && headerFile.size > 0) {
    if (headerFile.size > 16 * 1024 * 1024) {
      return NextResponse.json(
        { error: "A midia do template deve ter no maximo 16 MB." },
        { status: 400 }
      );
    }
    ...
    if (!inferredType) {
      return NextResponse.json(
        { error: "Nao foi possivel identificar o tipo da midia." },
        { status: 400 }
      );
    }
  ...
  if (!agent) {
    return NextResponse.json(
      { error: "Agente Lead/Meta nao encontrado ou inativo." },
      { status: 404 }
    );
  }
...
- Duplicados ignorados: ${importResult.duplicateRows}`,
  ...
  if (campaignError || !campaign) {
    return NextResponse.json(
      { error: campaignError?.message ?? "Nao foi possivel criar campanha de teste." },
      { status: 500 }
    );
  }
